...
class TagController extends ControllerBase
{
    ...
    public function showAction()
    {
        ...
        Tag::appendTitle(' | 所有标签 ');
        $this->assets->collection('tag_show')
    ...
    {
        Tag::appendTitle(' | 编辑标签 ');
        $this->assets->collection('tag_edit')
            ->addJs('js/jquery.dataTables.min.js')
            ->addJs('js/jquery-migrate-1.1.1.min.js')
            ->addJs('js/jquery.uniform.min.js');
        $this->assets->collection('tag_edit')
    ...
    public function saveAction()
    {
        ...
        if (!$tag) {
            $this->flash->error("标签不存在 " . $tag_name);
            return $this->dispatcher->forward(array(
                "controller" => "tag",
                "action" => "show"
            ));
